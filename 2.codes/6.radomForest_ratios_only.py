...
def randomForest_optimization(df_in, X, Y, Xtrain, Xtest, Ytrain, Ytest):
    ...
    df_grid_out.to_excel('../1.data_generated/6.1.rfc_results_ratios_only.xlsx', index=True)
    # save features
    ...
    features_df.to_excel('../1.data_generated/6.2.rfc_featureImportance_ratios_only.xlsx', index=False)
    # updated 2021-07-22
    # export ROC curve
    ...
    # plt.show()
    plt.savefig('../3.figures/6.1.ROC_rfc_ratios_only.pdf')
    plt.close()
    return features_df
...
def plot(features_df, n=15):
    ...
    plt.savefig('../3.figures/7.2.Barplot_rfc_Feature_Importance_ratios_only_%s.pdf'%str(datetime.date.today()))
    plt.close()
    return 0


def main():
    df_datatable_raw = pd.read_excel('../1.data_generated/1.ratio_datasheet_filtered_by_nonzero_and_std.xlsx', index_col=0)
    X, Y, Xtrain, Xtest, Ytrain, Ytest = dataset_seperation(df_in=df_datatable_raw)
    features_df = randomForest_optimization(df_datatable_raw, X, Y, Xtrain, Xtest, Ytrain, Ytest)
